...
@push('css')
    ...
    <style>
        ...
        .price-total .total-amount .currency {
            font-size: 0.85rem;
            font-weight: 700;
            margin-left: 4px;
        }
        ...
        .agree-box p.text-muted {
            font-size: 0.88rem;
            max-width: 500px;
            margin: 0 auto 20px;
        }

        .agree-box .form-check-input:checked {
        ...
        .track-widget .form-control:focus {
            border-color: #d4a843;
                        ...
                        <div class="price-card">
                            <div class="price-body">
                                ...
                                @elseif ($pricingType === 'formula')
                                    <div class="price-row"
                                        style="border-bottom:none;flex-direction:column;align-items:stretch;gap:8px">
                                        <span class="label" style="font-size:0.85rem">سعر المتر المربع</span>
                                        <span class="value"
                                            style="font-size:1.1rem">{{ number_format((float) ($settings['multiplier'] ?? 0), 2) }}
                                            ج.م / م²</span>
                                    </div>
                                    <div class="price-row">
                                ...
                                @elseif ($pricingType === 'tiered')
                                    ...
                                    <div class="price-row"
                                        style="border-bottom:none;flex-direction:column;align-items:stretch;gap:6px;padding-bottom:4px">
                                        <span class="label" style="font-size:0.85rem">جدول الشرائح</span>
                                        @foreach ($tiers as $tier)
                                            <div
                                                style="display:flex;justify-content:space-between;font-size:0.85rem;padding:3px 0">
                                                <span style="color:#636e72">حتى {{ number_format($tier['max']) }} م²</span>
                                                <span
                                                    style="font-weight:700;color:#0f1724">{{ number_format((float) ($tier['price'] ?? 0), 2) }}
                                                    ج.م</span>
                                            </div>
                                        @endforeach
                                        @if (empty($tiers))
                            ...
                            <div class="price-total">
                                ...
                                <div class="total-amount"><span
                                        class="currency">ج.م</span>{{ number_format($totalAmount, 2) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="row g-4">
                <div class="col-lg-8">
                    ...
                    <div class="agree-box mt-4">
                        ...
                        <p class="text-muted">يقر مقدم الطلب باطلاعه على الشروط والأحكام الفنية السابقة وصحة البيانات التي
                            سيدلي بها.</p>
                        <form action="{{ route('gis.apply.start', $service->slug) }}" method="GET">
                            <div class="form-check d-inline-block mb-3 text-start" style="padding-inline-start:2rem">
                                <label class="form-check-label" for="agreeCheck"
                                    style="cursor:pointer;font-weight:700">أوافق على جميع الشروط والأحكام</label>
                                <input class="form-check-input" type="checkbox" id="agreeCheck" required
                                    style="float: right;width:1.2rem;height:1.2rem;margin-inline-start:-2rem;margin-top:0.15rem;cursor:pointer">
                            </div>
                            <br>
                ...
                <div class="col-lg-4">
                    <div class="track-widget">
                        ...
                        <form action="{{ route('gis.tracking') }}" method="GET">
                            <input type="text" name="ticket" class="form-control mb-3"
                                placeholder="كود الشهادة / الطلب">
                            <button class="btn-navy w-100">تتبع الطلب <i class="fas fa-arrow-left"
